...
public class CutPerformer {
    public List<IntList> performCut(final MultiElement path, Cut cut) {
        return performSortedCuts(path, Collections.singletonList(cut));
    }
    ...
    public ArrayList<IntList> performSortedCuts(final MultiElement path, final List<Cut> cuts) {
        ...
        if (!cuts.isEmpty()) {
            Cut last = new Cut(path.getPoint(0), 0, 0);
            ...
            resultList.add(performCut(path, last, new Cut(path.getPoint(path.size() - 1), path.size() - 2, 1)));
        } else
    ...
    private static IntList performCut(final MultiElement path, final Cut previous, final Cut current) {
        ...
        for (int i = 1; i < size - 1; ++i)
            ret.add(path.getPoint(prevSegment + i));
