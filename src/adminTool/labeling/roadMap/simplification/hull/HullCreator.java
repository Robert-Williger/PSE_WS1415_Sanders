package adminTool.labeling.roadMap.simplification.hull;

import static adminTool.util.ShapeUtil.createStrokedPath;
...
public class HullCreator {
    ...
    public void createHulls(final List<Way> ways, final IPointAccess points) {
        ...
        final float strokeWidth = (float) info.getStrokeWidth(ways.get(0).getType());
    ...
    private IntList[] createIntersectionGraph(final List<Way> ways, final Area[] areas) {
        ...
        for (int u = 0; u < areas.length; ++u) {
            ...
            for (int v = u + 1; v < areas.length; ++v) {
                ...
                if (uWay.getPoint(0) == vWay.getPoint(0) || uWay.getPoint(uWay.size() - 1) == vWay.getPoint(0)
                        || uWay.getPoint(0) == vWay.getPoint(vWay.size() - 1)
                        || uWay.getPoint(uWay.size() - 1) == vWay.getPoint(vWay.size() - 1)) {
